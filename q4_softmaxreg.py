...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentence_i = [tokens[x] for x in sentence]
    sentVector = np.mean(wordVectors[sentence_i, :], axis=0)
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    prob = softmax(np.dot(features, weights))
    ...
    r = prob
    r[range(N), labels] -= 1.0
    grad = np.dot(features.transpose(), r) / N + regularization * weights
    #raise NotImplementedError
    ### END YOUR CODE
